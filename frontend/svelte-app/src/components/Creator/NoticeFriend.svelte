<script>
    ...
    async function GetNotice() {
        const response = await fetch(`${API_URL}/get_friend_notice`, {
            method: 'POST',
            headers: {
        ...
        if (data.status === 'success') {
            const {sender: fetchedsender, getter: fetchedgetter} = data.data;
    ...
    async function RespAdd() {
        const response = await fetch(`${API_URL}/resp_add_friend`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
<div class="deletor">
    <div class='block-header'><p>Приглашение в друзья:</p></div>
    <div class="data">
        {#if loading}
